...
def coolASCII():
	...
	print("\n".join(strings))
	print('Please wait while your bots are initialized\n')
	pass
		...
		for lines in dataFile:
			...
			#capture data
				...
				if balance != 0:
					...
					tempDict['amount'] = tempDict['amount'][:-1]
...
def roundHalf(self, x):
	if self.asset == 'btcusd':
		result = round(.5 * round(float(x)/.5),1)
		if result == 0:
			return 0.5
		else:
			return result
	elif self.asset == 'ethusd':
		result = round(round(.5 * round(float(x) * 10 / .5), 2) / 10, 2)
		if result == 0:
			return 0.5
		else:
			return result
	elif self.asset == 'eosusd':
		result = round(float(x), 3)
		if result == 0:
			return 0.001
		else:
			return result
	elif self.asset == 'xrpusd':
		result = round(float(x), 4)
		if result == 0:
			return 0.0001
		else:
			return result
...
def loadConfig():
	while True:
		try:
			configJSON = loadJsonFile('../config.json')
			expKeys = ['key','private','exchange','network','asset','version','supportResistance','leverageManager','stopTradingTrigger','killEmergencyTrigger','leverage','license']
			for keys in expKeys:
				if keys not in configJSON:
					print('CONFIG ERROR: ',keys, ' missing from config file')
					time.sleep(30)
					exit()
			
			if configJSON['exchange'] != 'bybit':
				print('CONFIG ERROR: exchange must be bybit')
				time.sleep(30)
				exit()
			
			if configJSON['network'] not in ['mainnet','testnet']:
				print('CONFIG ERROR: network must be mainnet or testnet')
				time.sleep(30)
				exit()
			
			if configJSON['asset'] not in ['btcusd','ethusd','eosusd','xrpusd']:
				print('CONFIG ERROR: asset must be btcusd ethusd eosusd or xrpusd')
				time.sleep(30)
				exit()
			
			if configJSON['version'].split('.')[0] != '2':
				print('CONFIG ERROR: please use version 2.0 or greater')
				time.sleep(30)
				exit()
			
			if 0 > int(configJSON['stopTradingTrigger']) or int(configJSON['stopTradingTrigger']) > 100 :
				print('CONFIG ERROR: stop trading trigger must be 0-100')
				time.sleep(30)
				exit()
				
			if 0 > int(configJSON['killEmergencyTrigger']) or int(configJSON['killEmergencyTrigger']) > 100 :
				print('CONFIG ERROR: kill emergency trigger must be 0-100')
				time.sleep(30)
				exit()
			
			if 0 > int(configJSON['leverage']) or int(configJSON['leverage']) > 100 :
				print('CONFIG ERROR: leverage must be 0-100')
				time.sleep(30)
				exit()
			...
			print('CONFIG ERROR: other error. Inspect config carefully')
			time.sleep(30)
			exit()
